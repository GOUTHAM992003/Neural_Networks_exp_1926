...
struct GPTConfig {
  ...
  int64_t n_embd = 768;
  int64_t n_layers = 6;
  int checkpoint_segments = 0;     // 0 to disable, > 0 to specify segments
  bool checkpoint_offload = false; // Whether to offload checkpoints to CPU
};
...
public:
  ...
  Tensor forward(const Tensor &idx) override {
    ...
    if (config.checkpoint_segments > 0) {
      x = autograd::checkpoint_sequential(mlps, config.checkpoint_segments, {x},
                                          config.checkpoint_offload)[0];
    } else {
      x = mlps->forward(x);
    }
...
int main() {
  try {
    ...
    config.checkpoint_segments = config.n_layers;   // Checkpoint every single layer
    config.checkpoint_offload = true; // Enable CPU offloading
    ...
    std::cout << "  Checkpoint Segments: " << config.checkpoint_segments
              << std::endl;
    std::cout << "  Checkpoint Offload: "
              << (config.checkpoint_offload ? "ENABLED" : "DISABLED")
              << std::endl;
    ...
    // Create CSV log file
    std::ofstream log_file("benchmark_training.csv");
    ...
    log_file.close();
    std::cout << "\nTraining log saved to: benchmark_training.csv"
              << std::endl;
