...
public:
    ...
    Embedding() = default;
    Embedding(int64_t vocab_size, int64_t embed_dim, DeviceIndex device, uint64_t seed = 42)
...
void init_linear_gpt2(nn::Linear& layer, float std = 0.02f, uint64_t seed = 42, bool req_grad=true) {
...
public:
    ...
    Attention(int64_t n_embd, int n_heads, int n_layers, DeviceIndex device, uint64_t seed = 42)
        : ln(n_embd),
    ...
    try {
        ...
        for (int step = start_step; step < MAX_STEPS; ++step) {
            ...
            if(step % TOK_GEN_FREQ == 0 || step == MAX_STEPS - 1) {
                ...
                std::vector<int64_t> xgen_tokens = {23337, 310,23337, 310,23337, 310,23337, 310,};
