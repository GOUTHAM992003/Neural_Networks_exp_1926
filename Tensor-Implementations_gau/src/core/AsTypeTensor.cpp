                ...
                dispatch_by_dtype(this->dtype_, [&](auto src_type_placeholder) {
                    ...
                    dispatch_by_dtype(new_dtype, [&](auto dst_type_placeholder) {
                        ...
                        if constexpr (src_is_complex == dst_is_complex) {
                            ...
                            for (size_t i = 0; i < n; ++i) {
                                dst_data[i] = static_cast<DstType>(src_data[i].real());
                            }
                        } else if constexpr (!src_is_complex && dst_is_complex) {
                            // Scalar -> Complex (set real part, imag=0)
                            for (size_t i = 0; i < n; ++i) {
                                // Cast to double to resolve ambiguity between int/float/double constructors
                                dst_data[i] = DstType(static_cast<double>(src_data[i]));
                            }
                        }
                    });
                });
