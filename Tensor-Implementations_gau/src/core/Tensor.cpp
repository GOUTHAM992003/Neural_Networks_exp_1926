...
namespace OwnTensor 
{
    Tensor::Tensor(Shape shape, Dtype dtype, DeviceIndex device, bool requires_grad)
        ...
        // Validate shape dimensions (0-dim tensor with empty dims is allowed like PyTorch)
        stride_.strides.resize(shape.dims.size());

        // Validate individual dimensions are positive (no empty or negative dims)
        for (size_t i = 0; i < shape_.dims.size(); ++i) 
        {
            if (shape_.dims[i] <= 0) 
            {
                throw std::runtime_error("All dimensions must be non-negative, and non-zero got dimension " + 
                                        std::to_string(i) + " = " + std::to_string(shape_.dims[i]));
            }
            // if (shape_.dims[i] == 0) 
            // {        
            //     throw std::runtime_error("Zero are not allowed, got dimension " + 
            //                             std::to_string(i) + " = 0");
            // }
        }
