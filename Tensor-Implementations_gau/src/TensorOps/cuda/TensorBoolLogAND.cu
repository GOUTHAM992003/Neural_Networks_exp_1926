#ifdef WITH_CUDA

#include <cuda_runtime.h>
#include <cuda_fp16.h>
#include <cuda_bf16.h>

#include "ops/TensorOps.cuh"
#include "core/Tensor.h"
#include "core/TensorDispatch.h"

namespace OwnTensor
{   
// FIXED: Convert to bool inline, then apply logical AND
template<typename T>
__global__ void bool_and_kernel(const T* a, const T* b, bool* output, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        // Convert to bool: 0 → false, non-zero → true
        bool a_bool = (a[idx] != T(0.0f));
        bool b_bool = (b[idx] != T(0.0f));
        output[idx] = a_bool && b_bool;
    }
}

// Specialization for __half (needs special comparison)
template<>
__global__ void bool_and_kernel<__half>(const __half* a, const __half* b, bool* output, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
        __half zero = __float2half(0.0f);
        bool a_bool = !__heq(a[idx], zero);  // true if not equal to zero
        bool b_bool = !__heq(b[idx], zero);
        output[idx] = a_bool && b_bool;
    }
}

// Specialization for __nv_bfloat16
template<>
__global__ void bool_and_kernel<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, bool* output, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
        __nv_bfloat16 zero = __float2bfloat16(0.0f);
        bool a_bool = !__heq(a[idx], zero);
        bool b_bool = !__heq(b[idx], zero);
        output[idx] = a_bool && b_bool;
    }
}

template<typename T>
__global__ void bool_and_kernel_broadcast(const T* a, const T* b, bool* output,
                                      const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                      const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                      size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    size_t a_bcast_strides[8];
    size_t b_bcast_strides[8];
    
    for (size_t i = 0; i < out_ndim; ++i) {
        size_t a_dim_idx = a_ndim - out_ndim + i;
        size_t b_dim_idx = b_ndim - out_ndim + i;
        
        if (a_dim_idx < a_ndim && a_shape[a_dim_idx] > 1) {
            a_bcast_strides[i] = a_strides[a_dim_idx];
        }
        if (b_dim_idx < b_ndim && b_shape[b_dim_idx] > 1) {
            b_bcast_strides[i] = b_strides[b_dim_idx];
        }
    }
    
    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = out_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % out_shape[dim];
        temp_idx /= out_shape[dim];
    }
    
    size_t a_idx = 0;
    size_t b_idx = 0;
    for (size_t dim = 0; dim < out_ndim; ++dim) {
        a_idx += coords[dim] * a_bcast_strides[dim];
        b_idx += coords[dim] * b_bcast_strides[dim];
    }
    
    // Convert to bool then apply logical AND
    bool a_bool = (a[a_idx] != T(0.0f));
    bool b_bool = (b[b_idx] != T(0.0f));
    output[linear_idx] = a_bool && b_bool;
}

// Specializations for broadcast kernels
template<>
__global__ void bool_and_kernel_broadcast<__half>(const __half* a, const __half* b, bool* output,
                                      const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                      const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                      size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    size_t a_bcast_strides[8] = {0};
    size_t b_bcast_strides[8] = {0};
    
    for (size_t i = 0; i < out_ndim; ++i) {
        size_t a_dim_idx = a_ndim - out_ndim + i;
        size_t b_dim_idx = b_ndim - out_ndim + i;
        
        if (a_dim_idx < a_ndim && a_shape[a_dim_idx] > 1) {
            a_bcast_strides[i] = a_strides[a_dim_idx];
        }
        if (b_dim_idx < b_ndim && b_shape[b_dim_idx] > 1) {
            b_bcast_strides[i] = b_strides[b_dim_idx];
        }
    }
    
    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = out_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % out_shape[dim];
        temp_idx /= out_shape[dim];
    }
    
    size_t a_idx = 0;
    size_t b_idx = 0;
    for (size_t dim = 0; dim < out_ndim; ++dim) {
        a_idx += coords[dim] * a_bcast_strides[dim];
        b_idx += coords[dim] * b_bcast_strides[dim];
    }
    
    __half zero = __float2half(0.0f);
    bool a_bool = !__heq(a[a_idx], zero);
    bool b_bool = !__heq(b[b_idx], zero);
    output[linear_idx] = a_bool && b_bool;
}

template<>
__global__ void bool_and_kernel_broadcast<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, bool* output,
                                      const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                      const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                      size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    size_t a_bcast_strides[8] = {0};
    size_t b_bcast_strides[8] = {0};
    
    for (size_t i = 0; i < out_ndim; ++i) {
        size_t a_dim_idx = a_ndim - out_ndim + i;
        size_t b_dim_idx = b_ndim - out_ndim + i;
        
        if (a_dim_idx < a_ndim && a_shape[a_dim_idx] > 1) {
            a_bcast_strides[i] = a_strides[a_dim_idx];
        }
        if (b_dim_idx < b_ndim && b_shape[b_dim_idx] > 1) {
            b_bcast_strides[i] = b_strides[b_dim_idx];
        }
    }
    
    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = out_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % out_shape[dim];
        temp_idx /= out_shape[dim];
    }
    
    size_t a_idx = 0;
    size_t b_idx = 0;
    for (size_t dim = 0; dim < out_ndim; ++dim) {
        a_idx += coords[dim] * a_bcast_strides[dim];
        b_idx += coords[dim] * b_bcast_strides[dim];
    }
    
    __nv_bfloat16 zero = __float2bfloat16(0.0f);
    bool a_bool = !__heq(a[a_idx], zero);
    bool b_bool = !__heq(b[b_idx], zero);
    output[linear_idx] = a_bool && b_bool;
}

// Host function remains the same
void cuda_logical_and_outplace(const Tensor &A, const Tensor &B, Tensor &output, cudaStream_t stream)
{
    bool needs_broadcasting = (A.shape().dims != B.shape().dims);
    size_t total_elems = output.numel();
    size_t block_size = 256;
    size_t grid_size = (total_elems + block_size - 1) / block_size;

    dispatch_by_dtype(A.dtype(), [&](auto dummy)
    {
        using T = decltype(dummy);
        const T* a_ptr = A.data<T>();
        const T* b_ptr = B.data<T>();
        bool* output_ptr = output.data<bool>();
        
        if (!needs_broadcasting) {
            bool_and_kernel<<<grid_size, block_size, 0, stream>>>(a_ptr, b_ptr, output_ptr, total_elems);
        } else {
            const auto& a_shape = A.shape().dims;
            const auto& b_shape = B.shape().dims;
            const auto& out_shape = output.shape().dims;
            
            size_t a_ndim = a_shape.size();
            size_t b_ndim = b_shape.size();
            size_t out_ndim = out_shape.size();
            
            size_t *d_a_shape, *d_b_shape, *d_out_shape;
            size_t *d_a_strides, *d_b_strides, *d_out_strides;
            
            cudaMalloc(&d_a_shape, a_ndim * sizeof(size_t));
            cudaMalloc(&d_b_shape, b_ndim * sizeof(size_t));
            cudaMalloc(&d_out_shape, out_ndim * sizeof(size_t));
            cudaMalloc(&d_a_strides, a_ndim * sizeof(size_t));
            cudaMalloc(&d_b_strides, b_ndim * sizeof(size_t));
            cudaMalloc(&d_out_strides, out_ndim * sizeof(size_t));
            
            cudaMemcpyAsync(d_a_shape, a_shape.data(), a_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_shape, b_shape.data(), b_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_out_shape, out_shape.data(), out_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_a_strides, A.stride().strides.data(), a_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_strides, B.stride().strides.data(), b_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_out_strides, output.stride().strides.data(), out_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            
            bool_and_kernel_broadcast<<<grid_size, block_size, 0, stream>>>(
                a_ptr, b_ptr, output_ptr,
                d_a_shape, d_b_shape, d_out_shape,
                d_a_strides, d_b_strides, d_out_strides,
                a_ndim, b_ndim, out_ndim, total_elems
            );
            
            cudaFree(d_a_shape);
            cudaFree(d_b_shape);
            cudaFree(d_out_shape);
            cudaFree(d_a_strides);
            cudaFree(d_b_strides);
            cudaFree(d_out_strides);
        }
    });
}

// ============================================================================
// IN-PLACE KERNELS
// ============================================================================

template<typename T>
__global__ void bool_and_inplace_kernel(T* a, const T* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
        bool a_bool = (a[idx] != T(0.0f));
        bool b_bool = (b[idx] != T(0.0f));
        // Write back as T (0.0f or 1.0f)
        a[idx] = static_cast<T>(a_bool && b_bool);
    }
}

template<>
__global__ void bool_and_inplace_kernel<__half>(__half* a, const __half* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
        __half zero = __float2half(0.0f);
        __half one = __float2half(1.0f);
        bool a_bool = !__heq(a[idx], zero);
        bool b_bool = !__heq(b[idx], zero);
        a[idx] = (a_bool && b_bool) ? one : zero;
    }
}

template<>
__global__ void bool_and_inplace_kernel<__nv_bfloat16>(__nv_bfloat16* a, const __nv_bfloat16* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
        __nv_bfloat16 zero = __float2bfloat16(0.0f);
        __nv_bfloat16 one = __float2bfloat16(1.0f);
        bool a_bool = !__heq(a[idx], zero);
        bool b_bool = !__heq(b[idx], zero);
        a[idx] = (a_bool && b_bool) ? one : zero;
    }
}

template<typename T>
__global__ void bool_and_inplace_kernel_broadcast(T* a, const T* b,
                                      const size_t* a_shape, const size_t* b_shape,
                                      const size_t* a_strides, const size_t* b_strides,
                                      size_t a_ndim, size_t b_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    // We need to map linear_idx (which iterates over A, since A is the output) 
    // to coordinates in A and B.
    // Since A is the output, linear_idx corresponds directly to A's layout if A is contiguous.
    // However, A might not be contiguous, but usually for element-wise ops we assume 
    // linear_idx maps to the logical output shape.
    // BUT: In-place ops require A to be the output.
    // If A is contiguous, linear_idx is the offset in A's data.
    // If A is NOT contiguous, we need to handle that. 
    // For simplicity, let's assume we iterate over the logical shape of A.
    
    // Calculate coordinates based on A's shape (which is the output shape)
    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = a_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % a_shape[dim];
        temp_idx /= a_shape[dim];
    }
    
    // Calculate physical offsets
    size_t a_idx = 0;
    size_t b_idx = 0;
    
    // For B, we need to handle broadcasting.
    // B's dimensions might be smaller or 1.
    // We align dimensions from the right.
    
    for (size_t i = 0; i < a_ndim; ++i) {
        a_idx += coords[i] * a_strides[i];
        
        // Map A's dim index to B's dim index
        // A has a_ndim, B has b_ndim.
        // dim i in A corresponds to dim j in B.
        int b_dim_idx = (int)b_ndim - (int)a_ndim + (int)i;
        
        if (b_dim_idx >= 0) {
             if (b_shape[b_dim_idx] == 1) {
                 // Broadcast: stride is effectively 0 (or we just don't add anything)
                 // But we should use the stride if it's 0? No, usually stride is 0 for broadcasted dims in some representations,
                 // but here we check shape.
                 // If shape is 1, index is always 0.
                 // b_idx += 0; 
             } else {
                 b_idx += coords[i] * b_strides[b_dim_idx];
             }
        }
    }
    
    bool a_bool = (a[a_idx] != T(0.0f));
    bool b_bool = (b[b_idx] != T(0.0f));
    a[a_idx] = static_cast<T>(a_bool && b_bool);
}

// Specializations for broadcast (Half/BFloat16) would follow similar pattern...
// For brevity, I will implement the generic one and rely on implicit conversion if possible,
// but for Half/BFloat16 we need explicit intrinsics.
// To save space/time, I'll implement the Half/BFloat16 specializations for broadcast now.

template<>
__global__ void bool_and_inplace_kernel_broadcast<__half>(__half* a, const __half* b,
                                      const size_t* a_shape, const size_t* b_shape,
                                      const size_t* a_strides, const size_t* b_strides,
                                      size_t a_ndim, size_t b_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = a_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % a_shape[dim];
        temp_idx /= a_shape[dim];
    }
    
    size_t a_idx = 0;
    size_t b_idx = 0;
    
    for (size_t i = 0; i < a_ndim; ++i) {
        a_idx += coords[i] * a_strides[i];
        int b_dim_idx = (int)b_ndim - (int)a_ndim + (int)i;
        if (b_dim_idx >= 0 && b_shape[b_dim_idx] != 1) {
             b_idx += coords[i] * b_strides[b_dim_idx];
        }
    }
    
    __half zero = __float2half(0.0f);
    __half one = __float2half(1.0f);
    bool a_bool = !__heq(a[a_idx], zero);
    bool b_bool = !__heq(b[b_idx], zero);
    a[a_idx] = (a_bool && b_bool) ? one : zero;
}

template<>
__global__ void bool_and_inplace_kernel_broadcast<__nv_bfloat16>(__nv_bfloat16* a, const __nv_bfloat16* b,
                                      const size_t* a_shape, const size_t* b_shape,
                                      const size_t* a_strides, const size_t* b_strides,
                                      size_t a_ndim, size_t b_ndim,
                                      size_t total_elems)
{
    size_t linear_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (linear_idx >= total_elems) return;

    size_t coords[8];
    size_t temp_idx = linear_idx;
    for (int dim = a_ndim - 1; dim >= 0; --dim) {
        coords[dim] = temp_idx % a_shape[dim];
        temp_idx /= a_shape[dim];
    }
    
    size_t a_idx = 0;
    size_t b_idx = 0;
    
    for (size_t i = 0; i < a_ndim; ++i) {
        a_idx += coords[i] * a_strides[i];
        int b_dim_idx = (int)b_ndim - (int)a_ndim + (int)i;
        if (b_dim_idx >= 0 && b_shape[b_dim_idx] != 1) {
             b_idx += coords[i] * b_strides[b_dim_idx];
        }
    }
    
    __nv_bfloat16 zero = __float2bfloat16(0.0f);
    __nv_bfloat16 one = __float2bfloat16(1.0f);
    bool a_bool = !__heq(a[a_idx], zero);
    bool b_bool = !__heq(b[b_idx], zero);
    a[a_idx] = (a_bool && b_bool) ? one : zero;
}

void cuda_logical_and_inplace(Tensor &A, const Tensor &B, cudaStream_t stream)
{
    bool needs_broadcasting = (A.shape().dims != B.shape().dims);
    size_t total_elems = A.numel();
    size_t block_size = 256;
    size_t grid_size = (total_elems + block_size - 1) / block_size;

    dispatch_by_dtype(A.dtype(), [&](auto dummy)
    {
        using T = decltype(dummy);
        T* a_ptr = A.data<T>();
        const T* b_ptr = B.data<T>();
        
        if (!needs_broadcasting) {
            bool_and_inplace_kernel<<<grid_size, block_size, 0, stream>>>(a_ptr, b_ptr, total_elems);
        } else {
            const auto& a_shape = A.shape().dims;
            const auto& b_shape = B.shape().dims;
            
            size_t a_ndim = a_shape.size();
            size_t b_ndim = b_shape.size();
            
            size_t *d_a_shape, *d_b_shape;
            size_t *d_a_strides, *d_b_strides;
            
            cudaMalloc(&d_a_shape, a_ndim * sizeof(size_t));
            cudaMalloc(&d_b_shape, b_ndim * sizeof(size_t));
            cudaMalloc(&d_a_strides, a_ndim * sizeof(size_t));
            cudaMalloc(&d_b_strides, b_ndim * sizeof(size_t));
            
            cudaMemcpyAsync(d_a_shape, a_shape.data(), a_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_shape, b_shape.data(), b_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_a_strides, A.stride().strides.data(), a_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_strides, B.stride().strides.data(), b_ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            
            bool_and_inplace_kernel_broadcast<<<grid_size, block_size, 0, stream>>>(
                a_ptr, b_ptr,
                d_a_shape, d_b_shape,
                d_a_strides, d_b_strides,
                a_ndim, b_ndim, total_elems
            );
            
            cudaFree(d_a_shape);
            cudaFree(d_b_shape);
            cudaFree(d_a_strides);
            cudaFree(d_b_strides);
        }
    });
}

}
#endif // WITH_CUDA