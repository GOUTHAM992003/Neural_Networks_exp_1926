...
template<typename T>
__global__ void bool_xor_inplace_kernel(T* a, const T* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
...
template<>
__global__ void bool_xor_inplace_kernel<__half>(__half* a, const __half* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
...
template<>
__global__ void bool_xor_inplace_kernel<__nv_bfloat16>(__nv_bfloat16* a, const __nv_bfloat16* b, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
...
template<typename T>
__global__ void bool_xor_inplace_kernel_broadcast(T* a, const T* b,
...
template<>
__global__ void bool_xor_inplace_kernel_broadcast<__half>(__half* a, const __half* b,
                                      const size_t* a_shape, const size_t* b_shape,
                                      const size_t* a_strides, const size_t* b_strides,
                                      size_t a_ndim, size_t b_ndim,
                                      size_t total_elems)
...
template<>
__global__ void bool_xor_inplace_kernel_broadcast<__nv_bfloat16>(__nv_bfloat16* a, const __nv_bfloat16* b,
                                      const size_t* a_shape, const size_t* b_shape,
                                      const size_t* a_strides, const size_t* b_strides,
                                      size_t a_ndim, size_t b_ndim,
                                      size_t total_elems)
...
void cuda_logical_xor_inplace(Tensor &A, const Tensor &B, cudaStream_t stream)
{
    bool needs_broadcasting = (A.shape().dims != B.shape().dims);
    ...
    dispatch_by_dtype(A.dtype(), [&](auto dummy)
    {
        ...
        if (!needs_broadcasting) {
            bool_xor_inplace_kernel<<<grid_size, block_size, 0, stream>>>(a_ptr, b_ptr, total_elems);
        } else {
            const auto& a_shape = A.shape().dims;
            const auto& b_shape = B.shape().dims;
            ...
            bool_xor_inplace_kernel_broadcast<<<grid_size, block_size, 0, stream>>>(
                a_ptr, b_ptr,
                d_a_shape, d_b_shape,
