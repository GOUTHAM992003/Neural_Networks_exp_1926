...
__global__ void bool_geq_kernel(const T* a, const T* b, bool* output, size_t n)
{
    ...
    if (idx < n)
    {
        if constexpr (is_complex_type_v<T>) { output[idx] = false; } else { output[idx] = gpu_ge(a[idx], b[idx]); }
    }
}
                                      ...
                                      size_t total_elems)
{
    ...
    if constexpr (is_complex_type_v<T>) { output[linear_idx] = false; } else { output[linear_idx] = gpu_ge(a[a_idx], b[b_idx]); }
}
