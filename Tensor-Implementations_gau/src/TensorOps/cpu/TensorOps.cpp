...
Tensor operator+(const Tensor& lhs, const Tensor& rhs) 
{
    ...
    {
        #ifdef WITH_CUDA
            cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
...
Tensor operator%(const Tensor& lhs, const Tensor& rhs) 
{
    ...
    if (lhs.device().is_cuda() && rhs.device().is_cuda())
    {
        #ifdef WITH_CUDA
            cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
            cuda_mod_tensor(lhs_promoted, rhs_promoted, output, stream);
        #else
            throw std::runtime_error("Tensor Ops: CUDA support not compiled");
        #endif
    }
        ...
        apply_binary_operation(lhs_promoted, rhs_promoted, output, [](auto a, auto b) {
            ...
            if constexpr (std::is_integral_v<T>) {
                ...
                return std::fmod(a, b);
                                 ...
                                 std::is_same_v<T, float8_e4m3fn_t> || std::is_same_v<T, float8_e5m2_t>) {
                return T(std::fmod(static_cast<float>(a), static_cast<float>(b)));
            } else {
                // Complex types: modulo not supported
                return T(0); 
            }
        });
    }
...
Tensor operator+=(Tensor& lhs, const Tensor& rhs)
{
    ...
    if (lhs.dtype() != rhs.dtype()) {
        ...
        Dtype promoted = promote_tensor_ops(lhs.dtype(), rhs.dtype());
...
Tensor operator%=(Tensor& lhs, const Tensor& rhs)
{
    ...
    if (lhs.device().is_cuda() && rhs.device().is_cuda())
    {
        #ifdef WITH_CUDA
            cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
            cuda_mod_tensor_inplace(lhs, rhs_converted, stream);
        #else
            throw std::runtime_error("Tensor Ops: CUDA support not compiled");
        #endif
    }
...
Tensor operator<=(const Tensor& lhs, const Tensor& rhs) 
{
    ...
    // Complex numbers are not ordered - throw error before processing
    if (promoted_dtype == Dtype::Complex32 || promoted_dtype == Dtype::Complex64 || promoted_dtype == Dtype::Complex128) {
        throw std::runtime_error("Complex numbers are not ordered, comparison operators (>,<,>=,<=) are not implemented.");
    }
    ...
    else 
    {
        apply_binary_op_bool(lhs_promoted, rhs_promoted, output, [](auto a, auto b) {
            ...
            if constexpr (std::is_same_v<T, complex32_t> || 
                ...
                return false; // Never reached due to early API check,as we r throwing error at api level itself above.
            } else {
                return a <= b;
            }
        });
    ...
    Tensor logical_XOR(const Tensor& lhs, const Tensor& rhs)
{
    ...
    Tensor& logical_AND_(Tensor& lhs, const Tensor& rhs)
    {
        // 1. Check broadcasting compatibility
        // Use broadcast_rhs_to_lhs to ensure rhs can be broadcast to lhs
        // without changing lhs shape.
        if (lhs.shape().dims != rhs.shape().dims) {
            broadcast_rhs_to_lhs(lhs.shape().dims, rhs.shape().dims);
        }

        if (lhs.dtype() != rhs.dtype()) {
            return logical_AND_(lhs, rhs.as_type(lhs.dtype()));
        }
        ...
        if (lhs.device().is_cuda() && rhs.device().is_cuda())
        {
            #ifdef WITH_CUDA
                cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
                cuda_logical_and_inplace(lhs, rhs, stream);
            #else
                throw std::runtime_error("Tensor Ops: CUDA support not compiled");
            #endif
        }
        else 
        {
            apply_binary_operation(lhs, rhs, lhs, [](auto a, auto b) {
                ...
                bool a_bool = (a != T1(0.0f));
                bool b_bool = (b != T2(0.0f));
                return T1(a_bool && b_bool);
            });
        }
        return lhs;
    }

    Tensor& logical_OR_(Tensor& lhs, const Tensor& rhs)
    {
        if (lhs.shape().dims != rhs.shape().dims) {
            broadcast_rhs_to_lhs(lhs.shape().dims, rhs.shape().dims);
        }
        ...
        if (lhs.device().is_cuda() && rhs.device().is_cuda())
        {
            #ifdef WITH_CUDA
                cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
                cuda_logical_or_inplace(lhs, rhs, stream);
            #else
                throw std::runtime_error("Tensor Ops: CUDA support not compiled");
            #endif
        }
    ...
    Tensor& logical_XOR_(Tensor& lhs, const Tensor& rhs)
    {
        if (lhs.shape().dims != rhs.shape().dims) {
            broadcast_rhs_to_lhs(lhs.shape().dims, rhs.shape().dims);
        }
        ...
        if (lhs.device().is_cuda() && rhs.device().is_cuda())
        {
            #ifdef WITH_CUDA
                cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
                cuda_logical_xor_inplace(lhs, rhs, stream);
            #else
                throw std::runtime_error("Tensor Ops: CUDA support not compiled");
            #endif
        }
    ...
    Tensor& logical_NOT_(Tensor& lhs)
    {
        // Unary op, no broadcasting check needed for shape (it's in-place)
        
        if (lhs.device().is_cuda())
        {
            #ifdef WITH_CUDA
                cudaStream_t stream = OwnTensor::cuda_1926::getCurrentStream();
                cuda_logical_not_inplace(lhs, stream);
            #else
                throw std::runtime_error("Tensor Ops: CUDA support not compiled");
            #endif
        }
