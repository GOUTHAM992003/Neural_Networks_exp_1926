...
__device__ __host__ inline uint16_t float_to_bfloat16(float f) {
    ...
    // Overflow check removed: bfloat16 shares the same exponent range as float32.
    // if (exponent > 0x8E) { ... } was incorrect (copy-paste from float16).
...
namespace OwnTensor {
     ...
     */
      struct complex32_t {
        ...
        // Bool constructor
        __device__ __host__ explicit complex32_t(bool b)
            : real_(b ? float16_t(1.0f) : float16_t(0.0f)), imag_(float16_t(0.0f)) {}
        // ---- Accessor Methods ----
        ...
        __device__ __host__ complex32_t& operator=(bool b) {
            real_ = b ? float16_t(1.0f) : float16_t(0.0f);
            imag_ = float16_t(0.0f);
            return *this;
        }
        ...
        // Dummy comparison operators for template instantiation (complex numbers are not ordered)
        // These should never be called at runtime due to early checks in TensorOps.cpp
    //     __device__ __host__ bool operator<(const complex32_t&) const { return false; }
    //     __device__ __host__ bool operator>(const complex32_t&) const { return false; }
    //     __device__ __host__ bool operator<=(const complex32_t&) const { return false; }
    //     __device__ __host__ bool operator>=(const complex32_t&) const { return false; }
     };
...
struct complex64_t {
    ...
    // Bool constructor
    __device__ __host__ explicit complex64_t(bool b)
        : real_(b ? 1.0f : 0.0f), imag_(0.0f) {}
    ...
    __device__ __host__ complex64_t& operator=(bool b) {
        real_ = b ? 1.0f : 0.0f;
        imag_ = 0.0f;
        return *this;
    }
    ...
    // // Dummy comparison operators for template instantiation (complex numbers are not ordered)
    // __device__ __host__ bool operator<(const complex64_t&) const { return false; }
    // __device__ __host__ bool operator>(const complex64_t&) const { return false; }
    // __device__ __host__ bool operator<=(const complex64_t&) const { return false; }
    // __device__ __host__ bool operator>=(const complex64_t&) const { return false; }
};
...
struct complex128_t {
    ...
    // Bool constructor
    __device__ __host__ explicit complex128_t(bool b)
        : real_(b ? 1.0 : 0.0), imag_(0.0) {}
    ...
    __device__ __host__ complex128_t& operator=(bool b) {
        real_ = b ? 1.0 : 0.0;
        imag_ = 0.0;
        return *this;
    }
    ...
    // // Dummy comparison operators for template instantiation (complex numbers are not ordered)
    // __device__ __host__ bool operator<(const complex128_t&) const { return false; }
    // __device__ __host__ bool operator>(const complex128_t&) const { return false; }
    // __device__ __host__ bool operator<=(const complex128_t&) const { return false; }
    // __device__ __host__ bool operator>=(const complex128_t&) const { return false; }
};
