    ...
    cudaStream_t stream)
{
    ...
    // ── 3d. Vectorized contiguous copy (float4/int4) when aligned ──
    // Only applies when the whole tensor is a single contiguous chunk after coalescing
    if (storage_offset == 0 && ndim == 1 && strides[0] == 1) {
        // Already handled by 3a (fully contiguous) above — but just in case:
        cudaMemcpyAsync(dst_bytes, src_bytes,
                        total_elems * elem_size,
                        cudaMemcpyDeviceToDevice, stream);
        return;
    ...
    // ── 3e. GENERIC FALLBACK: strided copy with FastDivmod ──
