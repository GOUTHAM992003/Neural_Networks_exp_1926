...
{
    // Unpack frequently-used fields once — aliases keep the kernel body identical
    // to the pre-strided version, only the per-row/batch address math changes.
    ...
    cudaStream_t stream = 0)
{
    ...
    // Build the strided-params struct once, reuse for every LAUNCH_SM89 variant.
