...
namespace OwnTensor 
{
    ...
    Tensor Tensor::contiguous() const {
        ...
        // If already contiguous with zero offset, return self — no allocation,
...
// int8_t (short)
    ...
    // std::vector<bool> is a bitset (no .data()), so we use uint8_t like fill<bool>.
    template<>
    void Tensor::fill_grad<bool>(bool value) {
        if (dtype() != Dtype::Bool) {
            throw std::runtime_error("Fill_grad bool: dtype must be Bool");
        }

        uint8_t fill_value = value ? 1 : 0;

        if (device().is_cpu()) {
            uint8_t* g = reinterpret_cast<uint8_t*>(grad());
            std::memset(g, fill_value, numel());
        } else {
            #ifdef WITH_CUDA
            std::vector<uint8_t> temp_data(numel(), fill_value);
