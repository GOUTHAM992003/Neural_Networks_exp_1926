...
#include "ops/LinearKernels.cuh"   // cuda_linear_bias_backward — coalesced reduce
#endif
#include <stdexcept>
#include <vector>
...
std::vector<Tensor> AddmmBackward::apply(std::vector<Tensor>&& grads) {
    ...
    // NOTE: tried routing to cuda_linear_bias_backward (LinearKernels.cu reduce_bias_kernel)
    // — measured 15x SLOWER per call than unified_reduce_kernel on this workload (r29).
    // The custom kernel needs algorithmic rework before it can replace this path.
    if (beta_ != 0.0f) {
        if (beta_ == 1.0f) {
