...
template<typename T>
bool launch_cublaslt_addmm(const Tensor& input, const Tensor& mat1, const Tensor& mat2, float alpha, float beta, Tensor& output, cudaStream_t stream, int M, int N, int K, int tb) {
    ...
    if (!out_contiguous) return false;
    if (!(a_contiguous || a_transposed)) return false;
    if (!(b_contiguous || b_transposed)) return false;
    ...
    int returned = 0;
    cublasLtMatmulAlgoGetHeuristic(handle, op_desc, layout_A, layout_B, layout_C, layout_C, pref, 1, &heuristic, &returned);

    float alpha_f = alpha;
    float beta_f = 0.0f; // Force 0 so C is completely overwritten with (A*B)+Bias

    void* workspace = get_workspace(dev_idx);
...
void cuda_addmm(const Tensor& input, const Tensor& mat1, const Tensor& mat2, float alpha, float beta, Tensor& output, cudaStream_t stream) {
    ...
    // --- cuBLASLt Fast Path for Bias ---
    if (beta == 1.0f && input.numel() == N) {
        bool success = false;
        dispatch_by_dtype(output.dtype(), [&](auto d) {
            using T = decltype(d);
            if constexpr (std::is_same_v<T, float> || std::is_same_v<T, float16_t> || std::is_same_v<T, __half> || std::is_same_v<T, bfloat16_t> || std::is_same_v<T, __nv_bfloat16>) {
                success = launch_cublaslt_addmm<T>(input, mat1, mat2, alpha, beta, output, stream, M, N, K, tb);
            }
        });
        if (success) {
            return;
