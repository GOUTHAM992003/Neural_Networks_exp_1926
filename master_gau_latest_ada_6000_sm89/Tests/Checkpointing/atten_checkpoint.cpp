...
struct GPTConfig {
  ...
  bool checkpoint_offload_to_cpu = false;
};
...
void init_linear_gpt2(nn::Linear &layer, float std = 0.02f, uint64_t seed = 42,
                      bool req_grad = true) {
...
public:
  ...
  Attention(int64_t n_embd, int n_heads, int n_layers, DeviceIndex device,
            uint64_t seed = 42)
  ...
  Tensor forward(const Tensor &x) override {
    ...
    Tensor attn_weights = autograd::matmul(autograd::mul(q, scale_),
...
public:
  ...
  MLP(int64_t n_embd, int n_layers, DeviceIndex device, uint64_t seed = 42)
...
public:
  ...
  GPT(GPTConfig cfg, DeviceIndex device, uint64_t seed = 42)
    ...
    for (int i = 0; i < cfg.n_layers; ++i) {
      auto a = std::make_shared<Attention>(
          cfg.n_embd, cfg.n_heads, cfg.n_layers, device, seed + 200 + i * 10);
    ...
    if (config.weight_tying) {
      ...
      lm_head = std::make_shared<nn::Linear>();
      {
        ...
        lm_head->weight =
            wte.weight.transpose(0, 1); // [n_embd, vocab_size] leaf view
      }
      lm_head->weight.set_requires_grad(
          true); // Ensure it's tracked by autograd
    } else {
  ...
  Tensor forward(const Tensor &idx) override {
    ...
    // std::cout << "Stats reserved before: " << stats.allocated_current / (1024
    // * 1024) << std::endl;
    ...
    for (int i = 0; i < config.n_layers; ++i) {
      if (config.use_checkpointing) {
        // Wrapper for a single transformer block (Attention + MLP)
        auto block_fn = [this,
                         i](const variable_list &inputs) -> variable_list {
          Tensor h = inputs[0];
          h = attn_blocks[i]->forward(h);
          h = mlp_blocks[i]->forward(h);
          return {h};
        };

        variable_list outputs = autograd::checkpoint(
            block_fn, {x}, config.checkpoint_offload_to_cpu);
        x = outputs[0];
      } else {
        // std::cout << "Transformer forward start: " << i << std::endl;
        x = attn_blocks[i]->forward(
            x); // pre-norm + multi-head attention + residual
    ...
    // std::cout << "Stats reserved after: " << stats.allocated_current / (1024
    // * 1024) << std::endl;
...
int main() {
  try {
    ...
    config.use_checkpointing = true; // Enable activation checkpointing
    config.checkpoint_offload_to_cpu = true;
    ...
    std::cout << "  head_dim: " << (config.n_embd / config.n_heads)
              << std::endl;
    ...
    if (!config.weight_tying) {
      std::cout << "(Note: More params than weight-tied version due to "
                   "separate lm_head)"
                << std::endl;
    }
    ...
    CheckpointManager ckpt_manager("checkpoints_gpt_att", "gpt_model", 5, 0, false, true);
    ...
    if (ckpt_manager.load_latest(model, optimizer, start_step, latest_loss)) {
      std::cout << "[Resume] Continuing from step " << start_step
                << " with loss " << latest_loss << std::endl;

      // Re-align dataloader: skip all batches consumed in steps 0...start_step
      size_t batches_to_skip =
          static_cast<size_t>(start_step + 1) * GRAD_ACCUM_STEPS;
      std::cout << "[Resume] Skipping " << batches_to_skip << " batches..."
                << std::endl;
    ...
    // Create CSV log file
    std::ofstream log_file("bluwerp_prompt.csv");
    ...
    for (int step = start_step; step < MAX_STEPS; ++step) {
      ...
      if (step % TOK_GEN_FREQ == 0 || step == MAX_STEPS - 1) {
        ...
        while (xgen.shape().dims[1] < max_length) {
          ...
          Tensor topk_probs_2d =
              OwnTensor::autograd::reshape(topk_probs, Shape({{B, 50}}));
          // topk_probs_2d.display();

          Tensor ix =
              Tensor::multinomial(topk_probs_2d, 1, /*replacement=*/false,
                                  /*seed=*/gen_seed++); // (B, 1)

          Tensor topk_indices_2d =
              OwnTensor::autograd::reshape(topk_indices, Shape({{B, 50}}));
      ...
      // lm_head->weight.grad is [n_embd, vocab_size] — transpose to
      // [vocab_size, n_embd] and add to wte.weight.grad so the optimizer sees
      ...
      nvtxRangePush("ckpt_manager.step");
