    ...
    int64_t rows, int64_t cols)
{
    ...
    // ── PASS 1: grad_gamma/grad_beta — OMP parallel reduction ──
    // Each thread accumulates into private buffers, then reduce
    const int max_threads = omp_get_max_threads();
    std::vector<float> gw_all(max_threads * cols, 0.0f);
    std::vector<float> gb_all(max_threads * cols, 0.0f);
    ...
    for (int64_t j = 0; j < cols; j += VEC) {
        ...
        sum_gb.storeu(grad_bias + j, count);
    }
    ...
    float eps)
{
    ...
    Tensor grad_bias = Tensor::zeros(grad_weight.shape(), wg_opts);
    ...
    if (grad_output.device().is_cuda()) {
        ...
        if (grad_output.dtype() == Dtype::Float32) {
            ...
            cuda::layer_norm_backward_cuda(
                ...
                mean.data<float>(), rstd.data<float>(), gamma,
                grad_input.data<bfloat16_t>(), gw_ptr, gb_ptr, rows, cols);
        }
    } else {
        if (grad_output.dtype() == Dtype::Float32) {
            ...
            cpu_layer_norm_backward_impl(
                grad_output.data<float16_t>(), input.data<float16_t>(),
                mean.data<float>(), rstd.data<float>(), gamma,
                grad_input.data<float16_t>(), grad_weight.data<float>(), grad_bias.data<float>(),
                rows, cols);
        } else if (grad_output.dtype() == Dtype::Bfloat16) {
            ...
            cpu_layer_norm_backward_impl(
                grad_output.data<bfloat16_t>(), input.data<bfloat16_t>(),
                mean.data<float>(), rstd.data<float>(), gamma,
                grad_input.data<bfloat16_t>(), grad_weight.data<float>(), grad_bias.data<float>(),
                rows, cols);
        }
    }
