...
variable_list CheckpointNode::apply(variable_list&& grads) {
    ...
    // 7. Run local backward (multi-root simulation)
    // Since Engine::backward only supports a single root, we iterate over outputs
    // and trigger backward for each one that has a valid gradient.
    // This works because gradients accumulate at the leaves (our recompute_inputs).
    
    // Note: This assumes standard autograd behavior where gradients accumulate.
    // A potential optimization would be to sum gradients for outputs that share the same grad_fn,
    // but typically outputs of a function all have the same grad_fn anyway.
    // Wait, if they are outputs of a forward function, they are likely leaves of the local graph 
    // produced by the LAST operation in the forward function.
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        if (outputs[i].requires_grad() && outputs[i].unsafeGetTensorImpl() && grads[i].unsafeGetTensorImpl()) {
             // We use the global backward function which handles the engine dispatch
             autograd::backward(outputs[i], &grads[i]);
        }
    }
